...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    ...
    echo '<input class="form-check-input" type="checkbox" name="payment_method[]" id="payment_' . htmlspecialchars($row['method_id']) . '" value="' . htmlspecialchars($row['method_name']) . '" onclick="updateSelectedOptions()">';
