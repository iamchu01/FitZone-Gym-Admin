...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $muscleCategory = $_POST['muscle_category'];  // This will be 'core' by default
    ...
    if (isset($_FILES['exercise_image']) && $_FILES['exercise_image']['error'] == 0) {
        ...
        $videoUploadDir = 'C:\\xampp\\htdocs\\sendtoal\\FitZone-Gym\\Videos\\abs';  // Updated directory for videos
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: abs-exercises.php"); // Replace with your exercises page URL
            exit;
        } else {
            echo "Error executing query: " . $stmt->error;
        }
            ...
            <div class="content container-fluid">
                <div class="page-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h3 class="page-title">Core Exercises</h3>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="targeted-exercise.php">Muscle Group</a></li>
                                <li class="breadcrumb-item active">Core Exercises</li>
                            </ul>
                        </div>
                ...
                <!-- Table displaying chest exercises -->
                <div class="panel-body">
                    ...
                    ?>
                          ...
                          <?php
                                    ...
                                    echo '<tr><td colspan="5">No exercises found for abs.</td></tr>';
                             }
                                ?>
                            </tbody>
                        </table>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addExerciseModalLabel">Add New Core Exercise</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"> </button>  
            </div>
            <form id="addExerciseForm" action="abs-exercises.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input 
                            ...
                            name="exercise_image" 
                            accept="image/*,video/*">
                        <small id="fileError" class="text-danger" style="display:none;">File name already exists. Please choose a different file.</small>
                    </div>
                </div>
...
document.getElementById('addExerciseForm').addEventListener('submit', function(event) {
    ...
    // Remove previous highlights and error messages
    const fields = document.querySelectorAll('.form-control');
    fields.forEach(field => {
        field.classList.remove('is-invalid');
    });
    document.getElementById('fileError').style.display = 'none';  // Hide file error message
    ...
    if (!exerciseImage.files.length) {
        ...
        valid = false;
        showError(exerciseImage, 'Please upload an image or video');
    } else {
        const file = exerciseImage.files[0];
        checkFileExistence(file.name).then(fileExists => {
            if (fileExists) {
                exerciseImage.classList.add('is-invalid');
...
function showError(element, message) {
    let existingErrorMessage = element.parentElement.querySelector('.invalid-feedback');
...
// Function to check if the file name already exists on the server
function checkFileExistence(fileName) {
    return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'check-file-existence.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function() {
            if (xhr.status === 200) {
                // Server should return "true" or "false"
                resolve(xhr.responseText.trim() === 'true');
            } else {
                reject('Error checking file existence');
            }
        };
...
addExerciseModal.addEventListener('hidden.bs.modal', function () {
    ...
    document.getElementById('fileError').style.display = 'none';  // Hide error message when modal closes
});
</script>
...
function openViewModal(name, description, mediaPath) {
    ...
    document.getElementById('viewMuscleCategory').value = 'core'; // Fixed muscle category
...
<div class="modal fade" id="editExerciseModal" tabindex="-1" aria-labelledby="editExerciseModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editExerciseModalLabel">Edit Core Exercise</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="editExerciseForm" action="update-exercise6.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
